...
**Description**: Prowl CLI installed for dogfooding on the marketing site: `prowl-tools@0.1.3` pinned as a devDependency (reproducible in CI, not a global install), `.prowl/` scaffolded via `prowl init` with config targeting `http://localhost:3002` and a starter `hello` hunt, Playwright Chromium provisioned during install to match the resolved Playwright version, and run artifacts (`.prowl/runs/`, `history.json`) gitignored while config and hunts stay committed. Verified end-to-end: `npx prowl run hello` passes 2/2 steps against the production build. Note: the CLI is repo-local — invoke as `npx prowl` (no global command). Newer CLI versions ship a single starter hunt, so no starter-hunt pruning was needed.
