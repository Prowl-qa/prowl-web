...
**Description**: The FinalCta "How it Works" link to `/cli#how-it-works` used a plain `<a>`, forcing a full document reload with no prefetch. Switched it to `next/link`'s `<Link>` for client-side navigation and prefetch. Same-page hash anchors elsewhere on the site were left as `<a>` intentionally. Lint + build green; `npm test` 26/26; full and mobile hunt suites green.

### ~~PQW-012: Newsletter form cleanup~~
**Resolved**: 2026-08-10 (commit 9f3c499, branch: medium-tier-cleanup)
**Description**: Re-scoped (owner decision): there was no live newsletter — the Buttondown embed action (`https://buttondown.com/api/emails/embed-subscribe/prowl`) pointed at an account we do not own, so the post-footer form silently misdirected/lost real subscriber emails. Removed the newsletter CTA block (heading, copy, and `<form>`) from `src/components/blog/PostFooter.tsx`, keeping the "Back to all posts" link intact. Updated the `blog` hunt in lockstep — the newsletter `scrollTo`/`visible`/`selectorExists` steps now assert the back-to-blog link (`a[href='/blog']`) instead. Restoring a newsletter once a real plan exists is tracked by PQW-023 (restore the block from git history). Verified via curl: the rendered blog post has zero `buttondown` strings and no `<form>` element, and the footer's back-to-blog link survives. Lint + build green; `npm test` 26/26; full and mobile hunt suites green.
