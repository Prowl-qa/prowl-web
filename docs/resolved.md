...
**Description**: Marketing sections previously shipped their SSR HTML behind motion/react's inline `opacity:0` `hidden` variant, so content — including the hero headline, the LCP element — was invisible until hydration and hidden entirely for no-JS visitors and crawlers. Added a shared `src/lib/reveal.ts` that renders content visible by default and treats the entrance animation as a progressive enhancement: `revealVisible` (`{ initial: false, animate: 'visible' }`) keeps above-the-fold heroes visible on first paint with no re-hide (correct for LCP; the trade-off is heroes no longer entrance-animate on first load), and `useScrollReveal` gates the `hidden` → `visible` scroll entrance on hydration via a `useSyncExternalStore` hook (no setState-in-effect) that also short-circuits to visible under `prefers-reduced-motion`. Because below-the-fold sections are off-screen when hydration runs, gating causes no flash and nothing is left permanently hidden. Applied across `Hero`, `SuiteHero`, `CodeReview`, `SectionReveal`, `TypingEffect`, and every `whileInView` section (Features, WhyProwl, DocsHub, WhyE2E, Community, ProductShowcase, AgentEfficiency). Verified: SSR HTML ships zero inline `opacity:0` on content across `/`, `/cli`, `/code-review`, `/docs`, and `/blog` (curl + grep); full and mobile hunt suites pass; lint and build green.
